...
$beach_id = isset($_GET['beach_id']) ? $_GET['beach_id'] : null;

// Fetch the beach data using PDO
if ($beach_id) {
...
// Check if the active beach ID is set
if (isset($_SESSION['active_beach_id'])) {
    $active_beach_id = $_SESSION['active_beach_id'];

    // Fetch data based on active beach
    $query = "SELECT * FROM beaches WHERE beach_id = :beach_id";
    $stmt = $db->prepare($query);
    ...
    if ($beach_data) {
        // Display data for the active beach
       // echo "Welcome to " . htmlspecialchars($beach_data['beach_name']);
        // Continue with other beach-specific content...
    } else {
        echo "<script>
                alert('Beach not found. Please check your selection.');
              </script>";
    }
} else {
    echo "<script>
            alert('No active beach selected. Please select your desired beach.');
          </script>";
}
?>
...
<body id="page-top">
        ...
        <div class="d-flex flex-column" id="content-wrapper">
            <div id="content">
                <nav class="navbar navbar-expand bg-white shadow mb-0 topbar">
                    <div class="container-fluid"><a class="navbar-brand d-flex align-items-center" href="#"><span>BRBMS</span></a><button class="btn btn-link rounded-circle me-3 shadow" type="button" data-bs-target="#offcanvas-menu" data-bs-toggle="offcanvas"><i class="fas fa-bars"></i></button></div>
                </nav>
                <section>
        ...
        <div class="offcanvas-body d-flex flex-column justify-content-between pt-0">
            <div>
                ...
                <ul class="nav nav-pills flex-column mb-auto">
                    ...
                    <li class="nav-item"><a class="nav-link link-body-emphasis" href="beach/manage.php?beach_id=<?= $beach['beach_id']; ?>"><svg xmlns="http://www.w3.org/2000/svg" width="1em" height="1em" fill="currentColor" viewBox="0 0 16 16" class="bi bi-umbrella-fill me-2">
                    ...
                    <li class="nav-item"><a class="nav-link link-body-emphasis" href="amenities/manage.php?beach_id=<?= $beach['beach_id']; ?>"><i class="fas fa-bed me-2"></i>&nbsp;Manage Amenities</a><a class="nav-link link-body-emphasis" href="reservation/manage.php?beach_id=<?= $beach['beach_id']; ?>"><svg xmlns="http://www.w3.org/2000/svg" width="1em" height="1em" fill="currentColor" viewBox="0 0 16 16" class="bi bi-calendar me-2" style="font-size: 17px;">
                                
                    <path d="M3.5 0a.5.5 0 0 1 .5.5V1h8V.5a.5.5 0 0 1 1 0V1h1a2 2 0 0 1 2 2v11a2 2 0 0 1-2 2H2a2 2 0 0 1-2-2V3a2 2 0 0 1 2-2h1V.5a.5.5 0 0 1 .5-.5M1 4v10a1 1 0 0 0 1 1h12a1 1 0 0 0 1-1V4z"></path>
                            </svg>&nbsp;Manage Reservations</a><a class="nav-link link-body-emphasis" href="transaction/manage.php?beach_id=<?= $beach['beach_id']; ?>"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 -32 576 576" width="1em" height="1em" fill="currentColor" class="me-2">
    ...
    <a class="dropdown-toggle link-body-emphasis d-flex align-items-center text-decoration-none" 
       ...
       role="button">
        <img class="rounded-circle me-2" alt="Beach Logo" width="32" height="32" 
             src="../assets/img/logo.jpg" style="object-fit: cover;">
        <strong>
            <?php
                // Check if an active beach name is set in the session, otherwise default to "Administrator"
                echo isset($_SESSION['active_beach_name']) ? htmlspecialchars($_SESSION['active_beach_name']) : 'Administrator';
            ?>
        </strong>&nbsp;
    </a>
    <ul class="dropdown-menu shadow text-small">
        ...
        // Fetch beaches owned by the user
        try {
            $query = "SELECT beach_id, beach_name FROM beaches WHERE user_id = :user_id";
            $stmt = $db->prepare($query);
            ...
            if (count($result) > 0) {
                foreach ($result as $row) {
                    echo '<li><a class="dropdown-item" href="../admin/beach/active_beach_id.php?beach_id=' 
                         . htmlspecialchars($row['beach_id']) . '">' 
                         . htmlspecialchars($row['beach_name']) . '</a></li>';
                }
            } else {
                echo '<li><span class="dropdown-item text-muted">No beaches found.</span></li>';
            }

        } catch (PDOException $e) {
            echo '<li><span class="dropdown-item text-danger">Error fetching beaches.</span></li>';
            error_log("Error: " . $e->getMessage());
        }
        ?>

        <div class="dropdown-divider"></div>
        <li><a class="dropdown-item" href="beach/add.html">Add New Beach</a></li>
